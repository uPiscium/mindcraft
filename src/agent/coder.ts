...
import { makeCompartment, initLockdown } from './library/lockdown';
...
export class Coder {
  ...
  async _lintCode(code: string): Promise<string | null> {
    ...
    const missingSkills = skills.filter((skill): skill is string => typeof skill === 'string' && !Object.prototype.hasOwnProperty.call(allDocs, skill));
  ...
  async _stageCode(code: string): Promise<{ func: { main: (bot: unknown) => Promise<unknown> | unknown }; src_lint_copy: string } | null> {
    ...
    const mainFn = compartment.evaluate(src) as (bot: unknown) => Promise<unknown> | unknown;
  ...
  _writeFilePromise(filename: string, src: string): Promise<string | null> {

...
import type { WorldBot } from "./world";
...
type InventoryItem = Item & { attackDamage?: number };
type PositionedBlock = Block & { position: Vec3 };
type PositionedEntity = Entity & { position: Vec3; name: string; metadata?: Record<number, unknown>; entityType?: number };
type ChestTrade = {
  disabled?: boolean;
  inputItem1: InventoryItem;
  inputItem2?: InventoryItem | null;
...
type ChestLike = {
  ...
  inputItem(): InventoryItem | null;
  outputItem(): InventoryItem | null;
  fuelItem(): InventoryItem | null;
  takeInput(): Promise<InventoryItem | null>;
  takeOutput(): Promise<InventoryItem | null>;
  takeFuel(): Promise<InventoryItem | null>;
...
type SkillsBot = Omit<WorldBot, 'inventory' | 'entities' | 'players' | 'modes' | 'pathfinder' | 'blockAt' | 'findBlocks' | 'nearestEntity' | 'recipesFor'> & {
  inventory: {
    items(): InventoryItem[];
    slots: Array<InventoryItem | null>;
  };
  recipesFor(itemId: number, meta: number | null, count: number, table: Block | boolean | null): Recipe[];
  craft(recipe: Recipe, count: number, table: Block | boolean | null): Promise<void>;
  openFurnace(block: Block): Promise<ChestLike>;
  openContainer(block: Block | Entity): Promise<ChestLike>;
  openVillager(villager: Entity): Promise<ChestLike>;
  trade(villagerInstance: ChestLike, tradeIndex: string | number, times?: number): Promise<void>;
  blockAt(position: Vec3): PositionedBlock | null;
  findBlocks(options: { matching: number | number[] | ((block: Block | null) => boolean); maxDistance: number; count: number }): Vec3[];
  nearestEntity(predicate: (entity: PositionedEntity) => boolean): PositionedEntity | null;
  pathfinder: pf.Pathfinder;
  tool: { equipForBlock(block: Block): Promise<void> };
  modes: { isOn(name: string): boolean; pause(name: string): void; unpause(name: string): void };
  players: Record<string, { entity?: PositionedEntity | null }>;
  entities: Record<string, PositionedEntity>;
  heldItem: InventoryItem | null;
  targetDigBlock: Block | null;
  bot?: never;
  activateItem(): Promise<void>;
  useOn(entity: Entity): Promise<void>;
  sleep(block: Block | null): Promise<void>;
  activateBlock(block: Block): Promise<void>;
  unequip(slot: string): Promise<void>;
  toss(itemType: number, metadata: null, count: number): Promise<void>;
  consume(): Promise<void>;
  dig(block: Block, disable?: boolean): Promise<void>;
  placeBlock(block: Block, face: Vec3 | null): Promise<void>;
  openChest(block: Block): Promise<ChestLike>;
  closeWindow(window: ChestLike): Promise<void>;
  lookAt(target: Vec3 | Block | Entity): Promise<void>;
  chat(message: string): void;
  blockAtCursor(maxDistance: number): Block | null;
  activateBlockAt?: never;
  registry: { enchantments: Record<string, { displayName: string }>; entitiesByName: { villager: { id: number } } };
};

const blockPlaceDelay = settings.block_place_delay == null ? 0 : settings.block_place_delay;
const useDelay = blockPlaceDelay > 0;

export function log(bot: SkillsBot, message: string) {
  bot.output += message + '\n';
}

async function autoLight(bot: SkillsBot) {
...
async function equipHighestAttack(bot: SkillsBot) {
...
export async function craftRecipe(bot: SkillsBot, itemName: string, num: number = 1) {
  /**
  ...
  let craftingTable: Block | boolean | null = null;
  ...
  placeTable: if (!recipes || recipes.length === 0) {
    ...
    if (craftingTable === null) {
      ...
      if (hasTable) {
        ...
        if (!pos) {
          log(bot, `Could not find space to place a crafting table.`);
          return false;
        }
  ...
  if (!recipes || recipes.length === 0) {
    const recipeInfo = mc.getItemCraftingRecipes(itemName)[0];
    const ingredients = recipeInfo ? Object.entries(recipeInfo[0] ?? {}) : [];
    log(bot, `You do not have the resources to craft a ${itemName}. It requires: ${ingredients.map(([key, value]) => `${key}: ${value}`).join(', ')}.`);
    if (placedTable) {
  ...
  if (craftingTable && craftingTable !== true && bot.entity.position.distanceTo(craftingTable.position) > 4) {
...
export async function wait(bot: SkillsBot, milliseconds: number) {
  /**
...
export async function smeltItem(bot: SkillsBot, itemName: string, num = 1) {
  /**
  ...
  if (!furnaceBlock) {
    ...
    if (hasFurnace) {
      ...
      if (!pos) {
        log(bot, `Could not find space to place a furnace.`);
        return false;
      }
  ...
  let total = 0;
  let smelted_item: InventoryItem | null = null;
...
export async function clearNearestFurnace(bot: SkillsBot) {
  /**
  ...
  let smelted_item: InventoryItem | null = null;
  let intput_item: InventoryItem | null = null;
  let fuel_item: InventoryItem | null = null;
  if (furnace.outputItem())
    smelted_item = await furnace.takeOutput();
  if (furnace.inputItem())
...
export async function attackNearest(bot: SkillsBot, mobType: string, kill: boolean = true) {
  /**
...
export async function attackEntity(bot: SkillsBot, entity: Entity, kill: boolean = true) {
  /**
   * Attack mob of the given type.
   * @param {MinecraftBot} bot, reference to the minecraft bot.
...
export async function defendSelf(bot: SkillsBot, range = 9) {
  /**
...
export async function collectBlock(bot: SkillsBot, blockType: string, num = 1, exclude: Vec3[] | null = null) {
  /**
...
export async function pickupNearbyItems(bot: SkillsBot) {
  /**
...
export async function breakBlockAt(bot: SkillsBot, x: number, y: number, z: number) {
  /**
...
export async function placeBlock(bot: SkillsBot, blockType: string, x: number, y: number, z: number, placeOn = 'bottom', dontCheat = false) {
  /**
...
export async function equip(bot: SkillsBot, itemName: string) {
  /**
...
export async function discard(bot: SkillsBot, itemName: string, num = -1) {
  /**
...
export async function putInChest(bot: SkillsBot, itemName: string, num = -1) {
  /**
...
export async function takeFromChest(bot: SkillsBot, itemName: string, num = -1) {
  /**
...
export async function viewChest(bot: SkillsBot) {
  /**
...
export async function consume(bot: SkillsBot, itemName = "") {
  /**
...
export async function giveToPlayer(bot: SkillsBot, itemType: string, username: string, num = 1) {
  /**
  ...
  if (await discard(bot, itemType, num)) {
    let given = false;
    bot.once('playerCollect', (collector: { username?: string }, collected: InventoryItem) => {
...
export async function goToGoal(bot: SkillsBot, goal: pf.goals.Goal) {
  /**
...
export async function goToPosition(bot: SkillsBot, x: number, y: number, z: number, min_distance = 2) {
  /**
...
export async function goToNearestBlock(bot: SkillsBot, blockType: string, min_distance = 2, range = 64) {
  /**
...
export async function goToNearestEntity(bot: SkillsBot, entityType: string, min_distance = 2, range = 64) {
  /**
...
export async function goToPlayer(bot: SkillsBot, username: string, distance = 3) {
  /**
...
export async function followPlayer(bot: SkillsBot, username: string, distance = 4) {
  /**
...
export async function moveAway(bot: SkillsBot, distance: number) {
  /**
...
export async function moveAwayFromEntity(bot: SkillsBot, entity: Entity, distance = 16) {
  /**
...
export async function avoidEnemies(bot: SkillsBot, distance = 16) {
  /**
...
export async function stay(bot: SkillsBot, seconds = 30) {
  /**
...
export async function useDoor(bot: SkillsBot, door_pos: Vec3 | null = null) {
  /**
...
export async function goToBed(bot: SkillsBot) {
  /**
...
export async function tillAndSow(bot: SkillsBot, x: number, y: number, z: number, seedType: string | null = null) {
  /**
...
export async function activateNearestBlock(bot: SkillsBot, type: string) {
  /**
 ...
 */
async function findAndGoToVillager(bot: SkillsBot, id: number | string) {
  id = id + "";
 ...
 */
export async function showVillagerTrades(bot: SkillsBot, id: number | string) {
  ...
  try {
    ...
    console.log('Villager trading error:', err instanceof Error ? err.message : String(err));
    return false;
  }
}
 ...
 */
export async function tradeWithVillager(bot: SkillsBot, id: number | string, index: number | string, count: number) {
  const villagerEntity = await findAndGoToVillager(bot, id);
  if (!villagerEntity) {
    return false;
  }
  ...
  try {
    ...
    try {
      ...
      console.log('Trade execution error:', tradeErr instanceof Error ? tradeErr.message : String(tradeErr));
      villager.close();
      return false;
    }
  } catch (err) {
    log(bot, 'Failed to open villager trading interface');
    console.log('Villager interface error:', err instanceof Error ? err.message : String(err));
    return false;
  }
}

function hasResources(window: Array<InventoryItem | null>, trade: ChestTrade, count: number) {
  const first = enough(trade.inputItem1, count);
  const second = !trade.inputItem2 || enough(trade.inputItem2, count);
  return first && second;

  function enough(item: InventoryItem, count: number) {
    let c = 0;
...
function stringifyTrades(bot: SkillsBot, trades: ChestTrade[]) {
...
function stringifyItem(bot: SkillsBot, item: InventoryItem | null) {
  ...
  if (item.nbt && item.nbt.value) {
    ...
    if (ench || StoredEnchantments) {
      text += ` enchanted with ${(ench || StoredEnchantments).value.value.map((e: { lvl: { value: number }; id: { value: string } }) => {
...
export async function digDown(bot: SkillsBot, distance = 10) {
  /**
...
export async function goToSurface(bot: SkillsBot) {
  /**
...
export async function useToolOn(bot: SkillsBot, toolName: string, targetName: string) {
  /**
...
export async function useToolOnBlock(bot: SkillsBot, toolName: string, block: Block) {
  /**

...
//     if (!isSpeaking) void processQueue().catch(err => { console.error('[TTS] processQueue error', err); isSpeaking = false; });
// }
...
//         url = await getModelUrl(prov);
//     } else {
//         prov = model.api;
//         mdl = model.model;
//         voice = model.voice;
//         url = model.url || await getModelUrl(prov);
//     }
...
//         //     $s.Speak('${txt.replace(/'/g,"''")}'); $s.Dispose()"` : isMac
//         //     ? `say "${txt.replace(/"/g,'\\"')}"`
...
//             isSpeaking = false;
//             void processQueue().catch(err => { console.error('[TTS] processQueue error', err); isSpeaking = false;
//         });
//     }
//     else {

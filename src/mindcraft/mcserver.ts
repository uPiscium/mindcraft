 ...
 */
export async function serverInfo(ip: string, port: number, timeout = 1000, verbose = false) {
 ...
 * @returns {Promise<Array>} - A Promise that resolves to an array of server info objects.
 */
export async function findServers(ip: string, earlyExit = false, timeout = 100) {
    const servers: any[] = [];
 ...
 */
export async function getServer(host: string, port: number, version: any) {
    let server: any = null;
    let serverString = "";
    let serverVersion = "";

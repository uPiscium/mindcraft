...
import * as mindcraft from './mindcraft.ts';
...
const agent_connections: Record<string, any> = {};
const agent_listeners: any[] = [];
...
class AgentConnection {
    ...
    full_state: any;
    viewer_port: number;
    constructor(settings: any, viewer_port: number) {
        this.socket = null;
...
export function registerAgent(settings: any, viewer_port: number) {
    let agentConnection = new AgentConnection(settings, viewer_port);
    agent_connections[settings.profile.name] = agentConnection;
}

export function logoutAgent(agentName: string) {
...
export function createMindServer(host_public = false, port = 8080) {
    ...
    io.on('connection', (socket: any) => {
        ...
        socket.on('create-agent', async (settings: any, callback: any) => {
        ...
        socket.on('get-settings', (agentName: string, callback: any) => {
            if (agent_connections[agentName]) {
        ...
        socket.on('disconnect', () => {
            if (agent_connections[curAgentName as string]) {
                console.log(`Agent ${curAgentName} disconnected`);
                agent_connections[curAgentName as string].in_game = false;
                agent_connections[curAgentName as string].socket = null;
                agentsStatusUpdate();
            }
        ...
        socket.on('chat-message', (agentName: string, json: any) => {
            if (!agent_connections[agentName]) {
        ...
        socket.on('set-agent-settings', (agentName: string, settings: any) => {
        ...
        socket.on('send-message', (agentName: string, data: any) => {
            if (!agent_connections[agentName]) {
        ...
        socket.on('bot-output', (agentName: string, message: any) => {
...
function agentsStatusUpdate(socket?: any) {
    if (!socket) {
        socket = io;
    }
    let agents: any[] = [];
...
let listenerInterval: any = null;
function addListener(listener_socket: any) {
                ...
                if (agent.in_game) {
                    try {
                        const state = await new Promise((resolve) => {
                            agent.socket.emit('get-full-state', (s: any) => resolve(s));
                        });
...
function removeListener(listener_socket: any) {

...
def run_task(task_path, task_id, profiles=None):
    """Run a single task using the Python entrypoint."""
    ...
    cmd = [sys.executable, main_path, "--task_path", task_path, "--task_id", task_id]
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all tasks in a JSON file sequentially"
    )
    ...
    parser.add_argument(
        "--delay", type=int, default=2, help="Delay in seconds between tasks"
    )

...
from mindcraft_py.config import resolve_settings
...
PYTHON_MAIN_PATH = os.path.join(PROJECT_ROOT, "main.py")


def build_main_command(task_path, task_id):
    normalized_task_path = task_path
    if not os.path.isabs(normalized_task_path):
        normalized_task_path = os.path.abspath(normalized_task_path)
...
def build_main_command_string(task_path, task_id):
    return shlex.join(build_main_command(task_path, task_id))
            ...
            if "turns" in data:
                for turn in data["turns"]:
                    if turn.get("role") == "system" and "content" in turn:
                        if (
                            isinstance(turn["content"], str)
                            and "Task ended with score : " in turn["content"]
                        ):
...
def check_folder_results(folder_path):
    ...
    if os.path.isdir(folder_path):
        subfolders = [
            f for f in glob.glob(os.path.join(folder_path, "*")) if os.path.isdir(f)
        ]
        if subfolders:
...
def read_settings(file_path):
    """Read and parse the shared settings file to get agent profiles."""
    _ = file_path
    profiles = resolve_settings().get("profiles", [])
...
def set_environment_variable_tmux_session(session_name, key, value):
    ...
    subprocess.run(
        ["tmux", "send-keys", "-t", session_name, f"export {key}={value}", "C-m"]
    )
...
def launch_parallel_experiments(
    ...
    if run_in_tmux:
        servers = create_server_files(
            "./tasks/server_data/", num_parallel, world_name=world_name
        )
    else:
        servers = [
            (f"./tasks/server_data_{i}/", 55916 + i) for i in range(num_parallel)
        ]
    ...
    while total_run < total_num_experiments:
        results = aggregate_results(
            [f"{experiments_folder}/{task_id}" for task_id in task_ids]
        )
        ...
        if s3:
            cmd = (
                f"aws s3 cp {experiments_folder}/results.txt s3://{s3_path}/results.txt"
            )
            print(cmd)
...
def launch_server_experiment(
    ...
    edit_file(
        os.path.join(server_path, "server.properties"), {"server-port": server_port}
    )
    ...
    if run_in_tmux:
        ...
        launch_world(
            server_path,
            session_name="server_" + session_name,
    ...
    if run_in_tmux:
        set_environment_variable_tmux_session(
            session_name, "MINECRAFT_PORT", server_port
        )
        set_environment_variable_tmux_session(
            session_name, "MINDSERVER_PORT", mindserver_port
        )
        set_environment_variable_tmux_session(
            session_name, "PROFILES", agent_profiles_str
        )
        set_environment_variable_tmux_session(
            session_name, "MAX_MESSAGES", str(max_messages)
        )
        set_environment_variable_tmux_session(
            session_name, "NUM_EXAMPLES", str(num_examples)
        )
        set_environment_variable_tmux_session(session_name, "LOG_ALL", "true")
        if insecure_coding:
            set_environment_variable_tmux_session(
                session_name, "INSECURE_CODING", "true"
            )
...
def run_script(
    ...
    for task_id in task_ids:
        ...
        cmd = build_main_command_string(task_path, task_id)
        cp_cmd = (
            f"cp {agent_names[0]}.json {server_path}bots/{agent_names[0]}/profile.json"
        )
        ...
        if s3:
            for agent in agent_names:
                script_content += (
                    f"aws s3 cp bots/{agent} s3://{s3_path}/bots/{agent} --recursive\n"
                )

    # Create a temporary shell script file
    script_file = f"./tmp/experiment_script_{session_name}.sh"
    make_script_file_and_run(
        script_content, script_file, session_name=session_name, run_in_tmux=run_in_tmux
    )
...
def make_ops(agent_names, session_name):
    ...
    cmd = build_main_command_string(
        "tasks/example_tasks.json",
        f"debug_{len(agent_names)}_agent_timeout",
    )
    ...
    subprocess.run(
        ["tmux", "send-keys", "-t", "server_" + session_name, f"/op @a", "C-m"]
    )

    agents_op = check_agent_ops(
        agent_names, ops_file=f"./tasks/server_data_{session_name}/ops.json"
    )
    if agents_op:
        print("Agents are operators! You are good to go :D")
    else:
        print(
            "Agents are not operators! We will need to try making them operators again!"
        )
        make_ops(agent_names, session_name)
...
def make_script_file_and_run(
    script_content, file_name, session_name="0", run_in_tmux=True
):
    ...
    if run_in_tmux:
        subprocess.run(
            ["tmux", "send-keys", "-t", session_name, script_file_run, "C-m"]
        )
    else:
...
def make_profiles(
    ...
    for index in range(len(agent_names)):
        ...
        if apis[index] == "vllm":
            profile["model"] = {"api": "vllm", "model": models[index], "url": url}
...
def create_server_files(source_path, num_copies, world_name="Forest"):
    ...
    for i in range(num_copies):
        ...
        print(dest_path)
        edit_file(
            dest_path + "server.properties",
            {"server-port": 55916 + i, "level-name": world_name},
        )
...
def copy_server_files(source_path, dest_path):
    ...
    if not same_files:
        copy_server_files(source_path, dest_path)
        print(
            "The destination path does not contain all the same files as the source path."
        )
    else:
...
def launch_world(
    server_path="./tasks/server_data/",
    agent_names=["andy", "jill"],
    session_name="server",
    port=55916,
):
...
def main():
    ...
    parser.add_argument(
        "--no_launch_world",
        action="store_true",
        help="Do not launch the Minecraft world",
    )
    parser.add_argument(
        "--task_path",
        default="tasks/multiagent_crafting_tasks.json",
        help="Path to the task file",
    )
    parser.add_argument(
        "--num_agents", default=2, type=int, help="Number of agents to run"
    )
    parser.add_argument(
        "--num_exp", default=1, type=int, help="Number of experiments to run"
    )
    parser.add_argument(
        "--num_parallel", default=1, type=int, help="Number of parallel servers to run"
    )
    ...
    parser.add_argument(
        "--bucket_name", default="mindcraft-experiments", help="Name of the s3 bucket"
    )
    parser.add_argument(
        "--add_keys",
        action="store_true",
        help="Create the keys.json to match the environment variables",
    )
    parser.add_argument(
        "--template_profile",
        default="profiles/tasks/crafting_profile.json",
        help="Model to use for the agents",
    )
    parser.add_argument(
        "--model", default="gpt-4o-mini", help="Model to use for the agents"
    )
    ...
    parser.add_argument(
        "--insecure_coding", action="store_true", help="Enable insecure coding"
    )
    ...
    parser.add_argument(
        ...
        default=15,
        type=int,
        help="Maximum number of messages before summarizing",
    )
    parser.add_argument(
        "--num_examples",
        default=2,
        type=int,
        help="Maximum number of turns before summarizing",
    )
    parser.add_argument(
        "--no-pruning", action="store_true", help="Disable pruning of the actions"
    )
    parser.add_argument(
        "--block_conversation", action="store_true", help="Block conversation actions"
    )
    parser.add_argument(
        "--check",
        metavar="FOLDER_PATH",
        help="Check and evaluate results in the specified folder without running experiments",
    )
    parser.add_argument(
        "--usernames",
        default="",
        help="Comma-separated list of usernames for the agents",
    )
    ...
    if "human_count" in task[list(task.keys())[0]]:
        ...
        if len(username_lst) != human_count:
            raise ValueError(
                f"Number of usernames provided ({len(username_lst)}) does not match human count ({human_count})"
            )
        if human_count > 0:
